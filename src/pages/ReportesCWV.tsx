...
export function ReportesCWV() {
  ...
  const fetchData = async () => {
    try {
      ...
      const reporteRes = await api.reportes.cwv.get({
  ...
  const handleRegenerar = async () => {
    try {
      setLoading(true)
      await api.reportes.cwv.regenerar({
        dominio_id: selectedDomain.id,
        mes,
        anio,
      })
    ...
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-4xl font-bold text-atom-dark">Reportes - Core Web Vitals</h1>
      ...
      {!loading && !reporte && (
        <div className="bg-yellow-50 border-l-4 border-yellow-500 p-6 rounded-lg">
          <p className="text-yellow-700">No hay datos de CWV para este período</p>
        </div>
      )}
    </div>
  )
